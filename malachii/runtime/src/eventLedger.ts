...
export interface LedgerEvent { id: string; sequence: number; type: string; timestamp: string; payload: unknown; prevHash: string; hash: string; }
export interface SignedCheckpoint { sequence: number; rootHash: string; signature: string; algorithm: "HMAC-SHA256"; }
...
export class EventLedger {
  ...
  constructor(seed: readonly LedgerEvent[] = []) {
    this.events = seed.map(e=>({...e}));
    if (!this.verify()) throw new Error("ledger_seed_integrity_failure");
  }
  ...
  snapshot(): readonly LedgerEvent[] { return this.events.map(e => ({...e})); }
  verify(events: readonly LedgerEvent[] = this.events): boolean {
    for (let i=0;i<events.length;i++) {
      const e=events[i]!; const expectedPrev=i===0?"GENESIS":events[i-1]!.hash;
      if (e.sequence!==i || e.prevHash!==expectedPrev) return false;
      const {hash,...base}=e; if (calc(base)!==hash) return false;
    } return true;
  }
  rootHash(): string { return this.events.length ? this.events[this.events.length-1]!.hash : "GENESIS"; }
}
